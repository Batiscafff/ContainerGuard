...
### Вхід та керування користувачами
...
1. Користувач реєструється на сторінці `/register`
2. Після реєстрації бачить сторінку `/pending` — очікує підтвердження
3. Адмін переходить до `/admin/users` і натискає **Активувати**
4. Користувач входить і отримує доступ до сканера

**Адмін-консоль** (`/admin/users`) дозволяє:
- Активувати / деактивувати акаунти
- Призначати або знімати права адміна
- Видаляти користувачів

На сторінці **Профіль** (`/profile`) доступний персональний API-ключ для роботи з JSON API.
...
| POST | `/login` | Вхід → redirect |
| GET | `/register` | Форма реєстрації |
| POST | `/register` | Створення акаунту → redirect /pending |
| GET | `/pending` | Очікування активації адміном |
| POST | `/logout` | Вихід |
| GET | `/profile` | Профіль + API-ключ |
| POST | `/profile/regenerate-key` | Перегенерувати API-ключ |
| GET | `/admin/users` | Адмін-консоль: список користувачів |
| POST | `/admin/users/{id}/activate` | Активувати акаунт |
| POST | `/admin/users/{id}/deactivate` | Деактивувати акаунт |
| POST | `/admin/users/{id}/promote` | Призначити адміном |
| POST | `/admin/users/{id}/demote` | Зняти права адміна |
